...
describe("1)  Test PartnerProgram #1)", () => {
    ...
    it("Deploy PartnerProgram", async function () {
        ...
        let percentReward = [10000000, 5000000];
        ...
        let INDEX_ROLE = await pProgram.INDEX_ROLE();
    ...
    it("Deploy IndexLP", async function () {
        ...
        const [
            ...
            ethers.getContractFactory('IndexLP'),
        ])
    ...
    it("DistributeTheReward", async function () {
        ...
        expect(await pProgram.getReferrer(addr[1].address)).eq('0x0000000000000000000000000000000000000000')
        ...
        await expect(pProgram.connect(addr[1]).setReferrer(addr[2].address)).revertedWith("You have referrer");

        console.log('balanceOf(addr[1].address=', await indexLP.balanceOf(addr[1].address))
        console.log('balanceOf(addr[2].address=', await indexLP.balanceOf(addr[2].address))

        await pProgram.connect(IndexAddr).distributeTheReward(addr[1].address, utils.parseEther('100'), indexLP.address)


        console.log('balanceOf(addr[2].address=', await indexLP.balanceOf(addr[2].address))
        console.log('balanceOf(addr[3].address=', await indexLP.balanceOf(addr[3].address))
